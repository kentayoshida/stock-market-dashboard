...
type UIDict = {
  ...
  sampleDow: string;
  sampleTopixCore30: string;
  sampleNifty50: string;
  trNote: string;
  // toggles
  periodAria: string;
  totalReturn: string;
...
export const ui: Record<Lang, UIDict> = {
  ja: {
    ...
    majorIndices: "主要指数",
    heroDow: "ダウ工業株30種",
    heroTopixCore30: "TOPIX Core30（主力30銘柄）",
    heroNifty50: "NIFTY 50（インド主要50銘柄）",
    ...
    coreAttribution:
      "「TOPIX Core30」等の指数値・商標は株式会社JPX総研またはその関連会社の知的財産です。本ページは指数そのものではなく、構成銘柄それぞれの個別株価（J-Quants・JPX公式API）に基づきます。",
    coreSourceNote:
      "構成銘柄は手動更新（TOPIX Core30 は定期的に入れ替えあり）。株価は J-Quants（JPX公式API・分割調整後終値）。配当を含まない価格リターンで、翌営業日更新のため1営業日ラグとなる場合があります。",
    niftyAttribution:
  ...
  en: {
    ...
    heroTopixCore30: "TOPIX Core 30",
    heroNifty50: "NIFTY 50",
    ...
    coreAttribution:
      "“TOPIX Core30” and related index values/trademarks are the intellectual property of JPX Market Innovation & Research, Inc. or its affiliates. This page is based on the individual share prices of the constituents (J-Quants, JPX official API), not the index itself.",
    coreSourceNote:
      "Constituents are updated manually (TOPIX Core30 is periodically rebalanced). Prices via J-Quants (JPX official API, split-adjusted close); price returns exclude dividends and may carry a 1-day lag as J-Quants updates on the next business day.",
    niftyAttribution:
...
// ---- TOPIX Core 30（4桁の東証コード → 英語社名）----
const CORE30_EN: Record<string, string> = {
  "2914": "Japan Tobacco",
...
// TOPIX Core 30: 4桁の東証コード → 表示名（en は英語社名）。
export function coreLabel(lang: Lang, code: string, jaLabel: string): string {
  if (lang === "en") return CORE30_EN[code] ?? jaLabel;
  return jaLabel;
}
